...
@section('content')
    <a href="{{ route('parent.announcements.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
    ...
    <x-card>
        ...
        @unless ($isRead)
            <form method="POST" action="{{ route('parent.announcements.read', $announcement) }}" class="mt-4">
                @csrf
