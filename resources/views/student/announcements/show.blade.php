...
@section('content')
    <a href="{{ route('student.announcements.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
    ...
    <x-card>
        ...
        @unless ($isRead)
            <form method="POST" action="{{ route('student.announcements.read', $announcement) }}" class="mt-4">
                @csrf
