...
@section('content')
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.timetable.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
            <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path d="M13 15l-6-5 6-5"/></svg>
            All timetable slots
        </a>
        <div class="flex items-center gap-2">
            <x-button :href="route('admin.timetable.edit', $slot)" variant="secondary" size="sm">Edit</x-button>
            <form method="POST" action="{{ route('admin.timetable.destroy', $slot) }}" onsubmit="return confirm('Remove this timetable slot?');">
                @csrf
                @method('DELETE')
