...
@section('content')
    <a href="{{ route('teacher.lesson-plans.index') }}" class="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
    ...
    @if ($lessonPlan->status === 'rejected' && $lessonPlan->review_note)
        <x-alert variant="danger">
            <span class="font-medium">Rejection reason:</span> {{ $lessonPlan->review_note }}
        </x-alert>
    @endif
    ...
    <x-card :padded="false">
        <dl class="divide-y divide-gray-100 text-sm">
            ...
            <div class="grid grid-cols-3 gap-4 px-4 py-3 sm:px-6">
                ...
                <dd class="col-span-2 text-gray-900">{{ $lessonPlan->created_at?->format('M j, Y g:ia') }}</dd>
            </div>
            @if ($lessonPlan->reviewed_by)
                <div class="grid grid-cols-3 gap-4 px-4 py-3 sm:px-6">
                    <dt class="text-gray-500">Reviewed by</dt>
                    <dd class="col-span-2 text-gray-900">
                        {{ $lessonPlan->reviewedBy?->name ?? '—' }}
                        @if ($lessonPlan->reviewed_at)
                            &middot; {{ $lessonPlan->reviewed_at->format('M j, Y g:ia') }}
                        @endif
                    </dd>
                </div>
            @endif
