...
class FeeControllerTest extends TestCase
{
    ...
    public function test_a_student_can_index_and_show_their_own_fee_metadata_without_balance_details(): void
    {
        $school = $this->makeSchool();
        ...
        $indexResponse->assertViewHas('fees', fn ($fees) => $fees->contains('id', $assessment->id));
        $indexResponse->assertDontSee('400.00');
        $indexResponse->assertDontSee('owed');
        ...
        $showResponse->assertViewMissing('amountRemaining');
        $showResponse->assertDontSee('400.00');
        $showResponse->assertDontSee('Amount remaining');
        $showResponse->assertDontSee('Payment history');
    }
    ...
    public function test_a_students_index_never_includes_another_students_assessment(): void
    {
        ...
        $response->assertViewHas('fees', function ($fees) use ($ownAssessment) {
            return $fees->count() === 1 && $fees->first()->id === $ownAssessment->id;
        });
    }
